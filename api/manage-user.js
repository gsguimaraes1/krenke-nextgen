...
// Consolida create/delete/resend-invite/set-password num único endpoint —
// Vercel Hobby limita 12 Serverless Functions por deploy, e cada api/*.js
// vira uma. Mesma lógica dos 4 endpoints que existiam antes, só roteada
// por `action` no body em vez de arquivo separado.
...
export default async function handler(req, res) {
  ...
  if (action === 'set-password') {
    ...
    if (!userId || !newPassword) return res.status(400).json({ error: 'userId e newPassword são obrigatórios' });
    if (newPassword.length < 6) return res.status(400).json({ error: 'A senha precisa ter no mínimo 6 caracteres' });
