...
const BlogPostPage: React.FC = () => {
    ...
    return (
        <article className="bg-slate-50 min-h-screen pt-24 pb-20">
            <Helmet>
                <title>{`${post.title} | Blog Krenke`}</title>
                <meta name="description" content={post.content.replace(/<[^>]*>?/gm, '').substring(0, 160)} />
                <link rel="canonical" href={`https://www.krenke.com.br/blog/${post.slug}`} />
                    ...
                    <div className="mt-20 pt-12 border-t border-gray-200">
                        ...
                        <div className="flex gap-4">
                            {[
                                ...
                                { icon: Twitter, color: 'bg-[#1DA1F2]', id: 'twitter' },
                                { icon: Linkedin, color: 'bg-[#0A66C2]', id: 'linkedin' },
                                { icon: LinkIcon, color: 'bg-gray-800', id: 'copy-link' }
                            ].map((social, i) => (
                                <button key={i} id={`btn-blog-share-${social.id}`} className={`${social.color} p-4 rounded-2xl text-white hover:scale-110 hover:-translate-y-1 transition-all shadow-lg`}>
                                    <social.icon size={20} />
                                </button>
                            ))}
                        </div>
