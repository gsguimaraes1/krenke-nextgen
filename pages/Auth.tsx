...
// Map common Supabase English error messages to Portuguese
...
const AuthPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(({ data: { session } }) => {
            if (session) navigate('/pgadmin', { replace: true });
        });
    }, [navigate]);
                ...
                <div className="bg-[#1a164d]/60 backdrop-blur-3xl rounded-[3rem] border border-white/10 p-10 md:p-14 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.5)] relative overflow-hidden">
                    ...
                    {mfaChallengeId ? (
                        ...
                        <form onSubmit={handleAuth} className="space-y-5">
                            ...
                            <button
                                id={isLogin ? 'btn-entrar' : 'btn-solicitar-cadastro'}
                                type="submit"
                                disabled={loading}
                                aria-label={isLogin ? 'Entrar no Sistema' : 'Solicitar Cadastro'}
